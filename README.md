...
Run **both** service migrations so the API gateway and user flows work against initialized databases:
...
Compose file: `infra/compose/docker-compose.dev.yml` with `infra/compose/.env.compose`. After `make up`, the gateway is at `http://127.0.0.1:8000` (built-in UI and `/v1` proxy); auth-service directly at `http://127.0.0.1:8001`.

### Password reset email (local SMTP)

The auth-service container loads [`services/auth-service/.env`](services/auth-service/.env) (generated by `bootstrap.sh` / `generate_dev_env.py`). To actually send reset emails locally, set SMTP there (do **not** commit real secrets):
...
| `SMTP_PASSWORD` | app password (16 characters, no spaces) |
| `SMTP_FROM_EMAIL` | same or authorized sender |
| `SMTP_FROM_NAME` | optional display name |
| `SMTP_USE_TLS` | `true` |

Set `AUTH_ALLOW_MISSING_SMTP=false` when you expect real delivery; if SMTP is missing and this is `true`, the API may return 202 without sending mail (enumeration-safe behavior).

Restart auth-service after editing `.env`:

```bash
docker compose --env-file infra/compose/.env.compose -f infra/compose/docker-compose.dev.yml up -d --force-recreate auth-service
```

Watch delivery in logs (no passwords are logged):

```bash
docker compose --env-file infra/compose/.env.compose -f infra/compose/docker-compose.dev.yml logs -f auth-service
```

Look for `email.sent` after a successful `POST /v1/auth/password/forgot`, or `email.smtp_failed` / `password_reset.email_send_failed` on errors.
