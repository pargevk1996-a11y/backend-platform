...
class FakeTwoFactorRepository:
    ...
    async def update_last_used_timecode(self, session, *, record: FakeSecretRecord, last_used_timecode: int):
        record.last_used_timecode = last_used_timecode
...
async def test_two_factor_setup_enable_and_backup_login() -> None:
    ...
    service = TwoFactorService(settings=settings, repository=repo, password_service=password_service)
